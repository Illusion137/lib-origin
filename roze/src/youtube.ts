...
export namespace RozeYouTube {
	...
	export async function get_all_transcript_infos(video_id: string){
		...
		if(info.captions?.caption_tracks === undefined) return generror("Couldn't retrieve transcripts for video", {video_id});
	...
	export async function get_transcript(transcript_info: TranscriptInfo, opts: BaseOpts){
		const transcript_response = await rozfetch(transcript_info.url + "&fmt=vtt", {...opts.fetch_opts, headers: {
            "User-Agent":
