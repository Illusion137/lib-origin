...
export namespace AudiobookGen {
    ...
    export async function roz_chapter_to_audiobook(roz_chapter: RozChapterContents, callbacks: RozChapterToAudiobookCallbacks = {}, voice_options: VoiceOptions = {}, clean_temp_files = true){
        ...
        for (const [i, roz_content] of roz_chapter.contents.entries()) {
            ...
            const content_temp_file_path = get_temp_file_path(Constants.TTS_DEFAULT_FILE_EXTENSION);
    ...
    export async function roz_full_audio(roz: Roz, callbacks: RozChapterToAudiobookCallbacks = {}, voice_options: VoiceOptions = {}, clean_temp_files = true){
    ...
    export async function roz_to_audiobook(roz: Roz, callbacks: RozChapterToAudiobookCallbacks = {}, voice_options: VoiceOptions = {}){
        const chapter_audiobooks = await Promise.all(roz.content.map(async(chapter) => roz_chapter_to_audiobook(chapter, callbacks, voice_options)));
