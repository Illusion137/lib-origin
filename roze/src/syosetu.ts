...
export namespace Syosetu {
    ...
    export async function webnovel_chapter_contents(webnovel_id: string, chapter: number, opts: Opts & { translate_contents?: boolean }): PromiseResult<WebnovelContents> {
        ...
        if (typeof series_name !== "string") return generror("Failed to find series_name", "CRITICAL", { webnovel_id, chapter, opts });
        ...
        if (author_element === null) return generror("Failed to find author_element", "CRITICAL", { webnovel_id, chapter, opts });
        ...
        if (typeof chapter_numbers_text !== "string") return generror("Failed to find chapter_numbers_text", "CRITICAL", { webnovel_id, chapter, opts });
        const chapter_numbers_split = chapter_numbers_text.split('/').map(v => Number(v)) as [number, number];
        if (chapter_numbers_split.length !== 2) return generror("chapter_numbers_split length !== 2", "CRITICAL", { webnovel_id, chapter, opts });

        let chapter_title = dom.querySelector(".p-novel__title")?.textContent;
        if (typeof chapter_title !== "string") return generror("Failed to find chapter_title", "CRITICAL", { webnovel_id, chapter, opts });

        const sections_of_lines_of_text = dom.querySelectorAll(".js-novel-text");
        if (sections_of_lines_of_text === undefined) return generror("Failed to find lines_of_text", "CRITICAL", { webnovel_id, chapter, opts });
