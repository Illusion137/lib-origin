...
switch (get_native_platform()) {
	case "WEB":
		console.error("Web Native MMKV is NOT implemented");
		break;
	case "ELECTRON_RENDERER":
		try {
			mmkv = require("../gen/modules/mmkv.electron_renderer").electron_renderer_mmkv;
		} catch (e) {}
		break;
	case "NODE":
		try {
			mmkv = require("./mmkv.node").node_mmkv;
		} catch (e) {}
		break;
	case "REACT_NATIVE":
		try {
			mmkv = require("./mmkv.mobile").mobile_mmkv;
		} catch (e) {}
		break;
}
...
export function generic_load_prefs<TType extends string, TKeys extends string>(mmkv_module: MMKVModule, prefs: BasePrefsRecord<TKeys, unknown, TType>, load_map: BasePrefLoadMap<BasePrefTypes | TType, unknown>) {
	...
	for (const key of keys) {
		...
		if (all_keys.includes(key) && load_function) prefs[key].current_value = load_function(mmkv_module, key);
...
export function generic_reset_prefs(mmkv_module: MMKVModule, keep_prefs: string[], load_prefs: () => void) {
	mmkv_module
		...
		.forEach((key) => mmkv_module.remove_key(key));
	load_prefs();
}
