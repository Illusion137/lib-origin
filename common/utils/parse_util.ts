...
export function try_json_parse<T>(json_string: string): T | ResponseError {
    try { return JSON.parse(json_string) as T; } catch (error) { return generror_catch(error, "Failed to parse JSON", "MEDIUM", { json_string: small_string(json_string) }); }
}

export function try_json_eval<T>(json_string: string): T | ResponseError {
    try { return json_eval<T>(json_string); } catch (error) { return generror_catch(error, "Failed to eval JSON", "MEDIUM", { json_string: small_string(json_string) }); }
}
