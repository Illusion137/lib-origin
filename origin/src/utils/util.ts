...
export function small_string(str: string){
...
export function try_json_eval<T>(json_string: string): T|ResponseError {
	try { return eval_json<T>(json_string); } catch (error) { return generror_catch(error, "Failed to eval JSON", {json_string: small_string(json_string)}); }
}
