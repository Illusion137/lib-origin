...
export namespace YouTubeDL {
    ...
    async function resolve_sabr_url_attempt(video_id: string): Promise<SabrTrackParams | ResponseError> {
        ...
        if (video_playback_ustreamer_config === undefined) return generror("ustreamerConfig not found", "MEDIUM", { video_id, player_response });

        const sabr_server_url = await client.session.player?.decipher(player_response.streaming_data?.server_abr_streaming_url);
        if (sabr_server_url === undefined) return generror("serverAbrStreamingUrl not found", "MEDIUM", { video_id, player_response });
