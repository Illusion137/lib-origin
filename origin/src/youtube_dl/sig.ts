 ...
 */
export const getFunctions = (html5playerfile: string, options: any) => exports.cache.getOrSet(html5playerfile, async() => {
...
const getExtractFunctions = (extractFunctions: any, body: string) => {
 ...
 */
export const setDownloadURL = (format: any, decipherScript: any, nTransformScript: any) => {
  ...
  const decipher = (url: string) => {
    ...
    components.searchParams.set((args.sp || 'sig') as string, decipherScript.runInNewContext(context));
 ...
 * @param {Object} options
 */
export const decipherFormats = async(formats: any[], html5player: string, options: any) => {
