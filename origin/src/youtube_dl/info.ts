...
const retryFunc = async (func, args, options) => {
  ...
  while (currentTry <= options.maxRetries) {
    try {
      ...
      if ((err as any)?.statusCode < 500 || currentTry >= options.maxRetries) throw err;
...
const parseJSON = (source, varName, json) => {
  if (!json || typeof json === "object") {
    ...
    try {
      ...
      throw Error(`Error parsing ${varName} in ${source}: ${(err as any).message}`);
    }
  }
};
...
export const getInfo = async (id, options) => {
  ...
  try {
    ...
    if (options.playerClients.includes("WEB")) {
      const bestPlayerResponse = info.player_response; bestPlayerResponse;
...
const playerAPI = async (videoId, payload, options) => {
  ...
  const response: any = await utils.request("https://youtubei.googleapis.com/youtubei/v1/player", opts);
