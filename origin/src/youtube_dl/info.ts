...
const fetchTvPlayer = async (videoId: string, info: Record<string, any>, options: DownloadOptions) => {
     ...
     (options as any).visitorId = getVisitorData(info, options);
...
const playerAPI = async (videoId: string, payload: object, options: any) => {
     ...
     if (options.visitorId) (opts.requestOptions.headers as any)["X-Goog-Visitor-Id"] = options.visitorId;
 ...
 */
const retryFunc = async (func: any, args: any, options: any) => {
...
const findJSON = (source: string, varName: string, body: string, left: string | RegExp, right: string, prependJSON: string) => {
...
const findPlayerResponse = (source: string, info: any) => {
...
const getWatchHTMLPage = async (id: string, options: any) => {
     const body = await getWatchHTMLPageBody(id, options);
     const info: any = { page: 'watch' };
     try {
          try {
...
const getDashManifest = (url: string, options: DownloadOptions): Promise<Record<string, AVFormat>> => new Promise((resolve, reject) => {
     const formats: Record<string, AVFormat> = {};
     ...
     utils.request(new URL(url, BASE_URL).toString(), options).then((res: any) => {
          parser.write(res);
          parser.close();
     }).catch(reject);
});
