...
const extractDecipherFunc = (body, _, code) => {
  try {
...
export const setDownloadURL = (format, decipherScript, nTransformScript) => {
  ...
  const decipher = url => {
    ...
    try {
      ...
      const decipheredSig = 
      runScript(context, decipherScript); 
      // decipherScript.runInNewContext(context);
  ...
  const nTransform = url => {
    try {
      ...
      const transformedN = 
        runScript(context, nTransformScript);
        // nTransformScript.runInNewContext(context);
