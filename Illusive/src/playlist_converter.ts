...
import { all_track_ids, music_service_track_primary_key } from "./illusive_utils";
...
export async function mutilate_to_illusi_playlist(convert_opts: ConvertTo, incoming_tracks: Track[], mode: MutilatePlaylistMode): MutilatePlaylistResponse {
    ...
    if (mode === "ADD") {
        ...
        if ("title" in convert_opts) {
            ...
            await SQLPlaylists.insert_all_tracks_playlist(resolved_track_uids.map(track_uid => ({ uuid: playlist_uuid, track_uid })));
        } else {
            await SQLPlaylists.insert_all_tracks_playlist(resolved_track_uids.map(track_uid => ({ uuid: convert_opts.uuid_uri, track_uid })));
        }
    } else { // REMOVE
        if ("title" in convert_opts) {
            const all_playlists = await SQLPlaylists.all_playlists_data();
