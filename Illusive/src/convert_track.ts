...
function convert_track_default_opts(opts: ConvertTrackOptsNull): ConvertTrackOpts{
    opts.to_music_service = opts.to_music_service ?? Prefs.get_pref('prefer_soundcloud') ? "SoundCloud" : Illusive.music_service.get("YouTube Music")!.has_credentials() ? "YouTube Music" : "YouTube";
    opts.deep_convert = opts.deep_convert ?? false;
    opts.proxies = opts.proxies ?? [];
    opts.possible_services = opts.possible_services ?? ["YouTube", "SoundCloud"];
    return opts as ConvertTrackOpts;
}

function is_youtube(track: Track){
