...
export async function illusi_startup(version: string, play_tracks: typeof GLOBALS.global_var.play_tracks, set_theme: typeof GLOBALS.global_var.set_theme, bottom_alert: typeof GLOBALS.global_var.bottom_alert, on_finish_essentials: () => {
    ...
    const phase = (name: string) => {
        ...
        timeline_mark(`phase ${name}`, { section_ms });
    };
    ...
    await catch_function_async(async () => {
        ...
        //     GLOBALS.global_var.past_playing_tracks = timeline_span_sync("past_queue restore", () => SQLTracks.add_playback_saved_data_to_tracks(Prefs.get_pref('past_queue').tracks));
        ...
        // phase("dir");
        // timeline_span("warmup_client", warmup_client).catch(catch_log);
        // phase("warmup");
        ...
        timeline_mark("illusi_startup body complete", { total_ms });
    ...
    // Chain-hop probe: "body complete" fires inside the wrapped fn, this fires
    // one await-resolution later. A gap between them means the promise machinery
    // itself was starved, not any startup statement.
    timeline_mark("illusi_startup resolved");
}
