...
export namespace Illusive {
    ...
    export function shuffle_tracks(mode: ShuffleMode, tracks: Track[], start_track?: Track): Track[] {
        switch (mode) {
            ...
            case "SHUFFLE": {
                const shuffled = Prefs.get_pref('use_track_shuffle_bias') ? FutsalShuffle.futsal_shuffle(tracks) : shuffle_array(tracks);
                if (start_track === undefined) return shuffled;
                const start_track_index = shuffled.findIndex(track => track.uid === start_track.uid);
                if (start_track_index === -1) return shuffled;
                [shuffled[0], shuffled[start_track_index]] = [shuffled[start_track_index], shuffled[0]];
                return shuffled;
            }
