...
export async function youtube_music_get_latest_release(id: string): Promise<CompactPlaylist|undefined>{
    ...
    const artist_albums_response = await YouTubeMusic.get_only_artist_albums({cookie_jar: get_cookie_jar('youtube_music_cookie_jar')}, artist_response.icfg.ytcfg, id);;
...
export async function apple_music_get_latest_release(id: string): Promise<CompactPlaylist|undefined>{
    return (await apple_music_get_artist(id)).latest_release;
}