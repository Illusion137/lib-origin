...
export namespace SQLUtils {
    export async function clean_directories_itemized(){
        ...
        const synced_lyrics_files    = error_undefined(await SQLfs.read_directory(SQLfs.synced_lyrics_directory(""))) ?? [];
        ...
        const synced_lyrics_uri_set = new Set(GLOBALS.global_var.sql_tracks.map(({synced_lyrics_uri}) => synced_lyrics_uri).filter(item => item !== undefined));
    ...
    export async function clean_directories() {
        ...
        files_to_delete.synced_lyrics.forEach(file => deletion_promises.push(SQLfs.delete_item(file)));
