...
import { convert_track, MaxTrack } from '../../convert_track';
...
export async function unzip_backpack(unavailable_tracks: Track[]): Promise<Track[]> {
    ...
    if(fastpack) {
        ...
        const promise_tracks: ReturnType<typeof convert_track>[] = [];
        ...
        for(const utrack of unavailable_tracks)
            promise_tracks.push(convert_track(utrack, {to_music_service: to_service, proxies}));
        ...
        for(let i = 0; i < resolved_tracks.length; i++) {
            ...
            (resolved_tracks[i] as MaxTrack).track!.uid = unavailable_tracks[i].uid;
        }
        return resolved_tracks.filter(track => !("error" in track)).map(item => (item as MaxTrack).track) as Track[];
    }
    const tracks: Track[] = [];
    for(const utrack of unavailable_tracks) {
        const track = await convert_track(utrack, {to_music_service: to_service});
        if("error" in track) {
            alert_error(track);
            continue;
        }
