...
export async function legacy_1307_track_to_track(legacy_1307_track: LegacyTypes1307.Track): Promise<Track> {
    ...
    const download_date: ISOString = media_info !== null && media_info.exists && !media_info.isDirectory ? new Date(media_info.modificationTime * 1000).toISOString() as ISOString : zero_iso;
