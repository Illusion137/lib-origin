-- Align soundcloud_id storage and RPC parsing with bigint-safe values.

ALTER TABLE tracks
    ALTER COLUMN soundcloud_id TYPE bigint USING soundcloud_id::bigint,
    ALTER COLUMN soundcloud_id SET DEFAULT 0;

DROP INDEX IF EXISTS tracks_soundcloud_id_idx;
CREATE UNIQUE INDEX IF NOT EXISTS tracks_soundcloud_id_idx
    ON tracks(soundcloud_id)
    WHERE soundcloud_id != 0;
...
DECLARE
    t jsonb;
    found_uid text;
    soundcloud_id_value bigint;
BEGIN
    FOR t IN SELECT * FROM jsonb_array_elements(tracks_json)
    LOOP
        found_uid := NULL;
        soundcloud_id_value := NULLIF(COALESCE(t->>'soundcloud_id', '0')::bigint, 0);
        ...
        WHERE tr.uid != (t->>'uid')
          AND (
            ...
            (soundcloud_id_value IS NOT NULL AND tr.soundcloud_id = soundcloud_id_value)                 OR
        ...
        IF found_uid IS NOT NULL THEN
            ...
            UPDATE tracks SET
                ...
                soundcloud_id        = CASE WHEN soundcloud_id = 0            THEN COALESCE(soundcloud_id_value, soundcloud_id)                           ELSE soundcloud_id END,
        ...
        ELSE
            ...
            INSERT INTO tracks (
                ...
                COALESCE(soundcloud_id_value, 0),
